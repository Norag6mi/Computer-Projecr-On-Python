...
# ============Content_under_frame1==================self.lbl_id = Label(self.frame1, text="Student ID",
bg="white", font=(
...
self.lbl_enter_marks=Label(self.frame1, text="1.Enter 
Course\n2.Enter Marks", bg="white", font=(
"times new roman", 10, "bold")).grid(row=2,column=0) 
self.combo_id =
ttk.Combobox(self.frame1,textvariable=self.combo_id_var, font=(
...
"times new roman", 15,
"bold"),textvariable=self.entry_roll_var, bg="azure2", 
state="readonly").grid(row=1, column=1,padx=5,pady=5)
self.course_id_combo=ttk.Combobox(self.frame1,textvariable=self.cours 
e_id_combo_var,font=("times new 
roman",10,"bold"),width=10,state="readonly",justify=RIGHT)
self.course_id_combo.place(x=120,y=75) 
self.marks_entry=Entry(self.frame1,
textvariable=self.entry_marks_var,validate="key",
validatecommand=(root.register(self.validate), '%P'),font=("times new 
roman", 15, "bold"),width=10, bg="azure2").place(x=215,y=75)
#===============Buttons-CRUD===================
self.btn1_crud=Button(self.frame1,text="Add",font=("Comic 
Sans MS", 10,
"bold"),activebackground="green",activeforeground="white",fg="black", 
command=self.crud_add).place(x=20,y=150)
self.btn2_crud=Button(self.frame1,text="Update",font=("Comic 
Sans MS", 10,
"bold"),activebackground="green",activeforeground="white",fg="black", 
command=self.update_crud).place(x=60,y=150)
self.btn3_crud=Button(self.frame1,text="Delete",font=("Comic 
Sans MS", 10,
"bold"),activebackground="green",activeforeground="white",fg="black", 
command=self.crud_delete).place(x=120,y=150)
self.btn4_crud=Button(self.frame1,text="Clear",font=("Comic 
Sans MS", 10,
"bold"),activebackground="green",activeforeground="white",fg="black", 
command=self.clear_crud).place(x=175,y=150)
self.search_image =
ImageTk.PhotoImage(ImageTk.Image.open("pictures_1/search_icon_2.jpg") 
.resize((40, 40), ImageTk.Image.ANTIALIAS))
self.reset_image=
ImageTk.PhotoImage(ImageTk.Image.open("pictures_1/reset_btn.jpg").res 
ize((40, 40), ImageTk.Image.ANTIALIAS))
self.search_entry=Entry(self.frame1,textvariable=self.txt_search,font 
=("times new roman",15,"bold"),bg="azure2").place(x=350,y=150)self.btn_search=Button(self.frame1,command=self.search_data,image=sel 
f.search_image,height=23,borderwidth=1,relief=GROOVE,bg="white")
self.btn_search.place(x=550,y=150) 
self.reset_btn = Button(self.frame1,
command=self.fetch_data,image=self.reset_image,
height=23,borderwidth=1,relief=GROOVE).place(x=590, y=150)
self.plot_scatter_btn=Button(self.frame1,command=lambda:
create_scatter_plot(r"csv_files/exam.csv"),text="Plot Scatter 
Plot",font=("times new roman", 10,
"bold"),justify=CENTER,bg="red",activebackground="yellow",activeforeg 
round="blue").place(x=350,y=100)
#======Treeview==============================
scroll_x=ttk.Scrollbar(self.frame2,orient=HORIZONTAL) 
scroll_y = ttk.Scrollbar(self.frame2, orient=VERTICAL) 
self.style=ttk.Style()
self.style.theme_use("clam")
self.style.configure("Treeview",background="grey71",foreground="black 
",rowheight=25,fieldbackground="grey71")
self.style.map("Treeview",background=[("selected","green")]) 
self.exam_headings=pd.read_csv("csv_files\exam.csv")
self.exams_Table=ttk.Treeview(self.frame2,columns=list(self.exam_head 
ings.columns),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set 
)
...
if self.entry_marks_var.get()=="" or
self.combo_id_var.get()=="" or self.course_id_combo_var.get()=="":
messagebox.showerror("Error","Entry bars should not be 
...
if (self.df.loc[i, self.exam_headings.columns[1]]
...
messagebox.showerror("Error", "Entry box shouldn't be empty.",
parent=self.root)
else:
try:
df_filtered =
self.exam_headings.loc[self.exam_headings[self.exam_headings.columns[ 
0]] == self.txt_search.get(
)]
df_rows = df_filtered.to_numpy().tolist()
...
self.df[self.exam_headings.columns[1]] ==
...
df=df.drop_duplicates(subset=[self.exam_headings.columns[0],self.exam 
_headings.columns[1]])
...
messagebox.showinfo("Info","Marks Obtained should 
be less than or equal to 100",parent=self.root)
else:
df = pd.read_csv("csv_files\exam.csv") 
data={self.exam_headings.columns[0]:
[self.course_id_combo_var.get()], self.exam_headings.columns[1]: [
self.combo_id_var.get()],
self.exam_headings.columns[2]: [self.entry_marks_var.get()]}
