...
self.root.geometry("1000x500+{}+{}".format(int(x)+100, 
int(y)))
...
# =======frame1_content============
font=(
self.lbl_id = Label(self.frame1, text="Batch ID", bg="white",
"times new roman", 10, "bold")).grid(row=0, column=0) 
self.lbl_name = Label(self.frame1, text="Batch Name",
bg="white", font=(
"times new roman", 10, "bold")).grid(row=1, column=0) 
self.lbl_course_lst = Label(self.frame1, text="List of 
Courses", bg="white", font=(
"times new roman", 10, "bold")).grid(row=2, column=0) 
self.combo_id = ttk.Combobox(self.frame1,
textvariable=self.combo_id_var, font=(
"times new roman", 15, "bold"), state="readonly",
justify=RIGHT)
self.combo_id.grid(row=0, column=1, padx=2, pady=2) 
self.entry_name = Entry(self.frame1,
textvariable=self.entry_name_var, font=(
"times new roman", 15, "bold"), bg="azure2").grid(row=1,
column=1, padx=5, pady=5)
self.listbox = Listbox(self.frame1, bg="azure3",
highlightcolor="orange", width=33,
height=5)
self.listbox.grid(row=2, column=1) 
self.entry_course = Entry(self.frame1,
textvariable=self.course_var, font=(
"times new roman", 15, "bold"), bg="azure2").grid(row=3,
column=1, padx=5, pady=5)
# ================RELOAD button================
self.reset_image = ImageTk.PhotoImage(ImageTk.Image.open("pictures_1/reset_btn.jpg").resize((40, 40),
ImageTk.Image.ANTIALIAS))
self.reset_btn = Button(self.frame1, image=self.reset_image,
height=23, borderwidth=1,
relief=GROOVE,
command=self.update_combobox()).place(x=310, y=5)
self.btn1 = Button(self.frame1, text="Add", justify=CENTER,
command=self.add_course).grid(row=2,
column=3,)
self.btn2 = Button(self.frame1, text="Delete", 
justify=CENTER,
command=lambda:
self.listbox.delete(ANCHOR)).grid(row=2, column=4,)
self.text_std = Text(self.frame1, height=10, width=50,
bg="yellow")
self.text_std.place(x=390, y=10)
self.btn_pie = Button(self.frame1, text="Plot Piechart",
justify=CENTER,command=lambda:self.display_piechart(self.combo_id_var 
.get())).place(x=800, y=10)
self.btn1_crud = Button(self.frame3, text="Add", font=("Comic 
Sans MS", 10, "bold"), activebackground="green",
activeforeground="white",
relief=RIDGE, bd=2, command=self.crud_add).pack(side=TOP, fill=X)
self.btn2_crud = Button(self.frame3, text="Update",
font=("Comic Sans MS", 10, "bold"), activebackground="green",
activeforeground="white",
relief=RIDGE, bd=2, command=self.update_crud).pack(side=LEFT)
self.btn3_crud = Button(self.frame3, text="Delete",
font=("Comic Sans MS", 10, "bold"), activebackground="green",
activeforeground="white",
relief=RIDGE, bd=2, command=self.crud_delete).pack(side=RIGHT)
self.btn4_crud = Button(self.frame3, text="Clear",
font=("Comic Sans MS", 10, "bold"), activebackground="green",
activeforeground="white",
relief=RIDGE, bd=2, command=self.clear_crud).pack(side=BOTTOM)
self.lbl_search_by = Label(self.frame1, text="Search By 
BatchID", font=(
"Comic Sans MS", 10, "bold"), bg="white").place(x=800,
y=125)
self.search_image = ImageTk.PhotoImage(ImageTk.Image.open( 
"pictures_1/search_icon_2.jpg").resize((40, 40),
ImageTk.Image.ANTIALIAS))
self.search_entry = Entry(self.frame1,
textvariable=self.txt_search, font=(
"times new roman", 10, "bold"), bg="azure2").place(x=800,
y=150)
self.btn_search = Button(self.frame1,
image=self.search_image, height=23,
borderwidth=1, relief=GROOVE,
bg="white", command=self.search_data)
self.btn_search.place(x=880, y=170)self.show_allbtn = Button(self.frame1,
command=self.fetch_data, text="Show All", width=10,
height=0, pady=1, bg="OrangeRed3",
font=("times new roman", 10, "bold")).place(x=800, y=170)
...
fieldbackground="grey71")
self.style.map("Treeview", background=[("selected", 
"green")])
...
def add_course(self):if self.course_var.get() != "":
...
== "" or list(self.listbox.get(0, END)) == []:
messagebox.showerror(
"Error", "Entry bars should not be empty",
parent=self.root) 
else:
try:
excel_filename = r"csv_files\Batches.csv" 
...
self.df[self.batches_headings.columns[0]] ==
self.combo_id_var.get())], axis=0, inplace=True)
...
if self.combo_id_var.get() == "" or self.entry_name_var.get() 
== "" or list(self.listbox.get(0, END)) == []:messagebox.showerror(
"Error", "Entry bars should not be empty",
parent=self.root) 
else:
try:
...
if (self.df.loc[i,
self.batches_headings.columns[0]] == self.combo_id_var.get()):
print("Hello world") 
self.df.loc[i,
self.batches_headings.columns[1]
] = self.entry_name_var.get()
self.df.loc[i,
self.batches_headings.columns[3]] = list(
...
self.text_std.insert(END, tabulate(self.student_lst,
tablefmt="grid"))
def display_piechart(self,batchid):
student_lst = self.student_lister(batchid) 
pct_list=self.return_lst_ofmarks(student_lst)student_lst["Percentages(%)"]=pct_list
filtered_rows=student_lst.loc[:,[student_lst.columns[0],student_lst.c 
olumns[2]]]
studentID = [] 
marks = []
...
#create pie chart
plt.pie(marks, labels = studentID, colors = colors, 
autopct='%.0f%%')
...
messagebox.showerror(
"Error", "Entry box shouldn't be empty.",
parent=self.root) 
else:
try:
df_filtered =
self.batches_headings.loc[self.batches_headings[self.batches_headings 
.columns[0]] == self.txt_search.get(
)]
...
try:
excel_filename = r"csv_files\batches.csv" 
df = pd.read_csv(excel_filename)if self.combo_id_var.get() not in list(df.loc[:,
self.batches_headings.columns[0]]):
batch_ids = self.find_students_in_batch( 
self.combo_id_var.get())
data = {self.batches_headings.columns[0]:
[self.combo_id_var.get()], self.batches_headings.columns[1]: [
self.entry_name_var.get()],
self.batches_headings.columns[2]: [self.combo_id_var.get()[:-2]],
self.batches_headings.columns[3]: [list(self.listbox.get(0, END))],
self.batches_headings.columns[4]: [batch_ids]}
...
messagebox.showerror(
"Error", "Department ID should be unique.",
parent=self.root)
except Exception as es: 
messagebox.showerror(
"Error", f"Error due to: {str(es)}",
...
return
list(itertools.chain(*filtered_df.to_numpy().tolist())) 
